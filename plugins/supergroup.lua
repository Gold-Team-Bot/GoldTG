...
local function check_member_super(cb_extra, success, result)
  ...
  for k,v in pairs(result) do
    ...
    if member_id ~= our_id then
      ...
      save_data(_config.moderation.data, data)
	  local text = '✔️ گروه شما به لیست گروه های مدیریتی ربات اضافه شد ✔️'
...
local function check_member_superrem(cb_extra, success, result)
  ...
  for k,v in pairs(result) do
    ...
    if member_id ~= our_id then
      ...
      save_data(_config.moderation.data, data)
	  local text = '❌ گروه از لیست گروه های مدیریتی ربات حذف شد ❌'
      return reply_msg(msg.id, text, ok_cb, false)
    end
  end
end
...
local function unlock_group_links(msg, data, target)
  ...
  local group_lang = redis:hget(hash,'lang')
  if group_lang then
    return reply_msg(msg.id,'🔒قفل لینک درسوپر گروه غیرفعال شد🔓', ok_cb, false)
     else 
	...
	local data = load_data(_config.moderation.data)
		if matches[1] == 'add' and not matches[2] then
			...
			if is_super_group(msg) then
				return reply_msg(msg.id, '🔘گروه شما به لیست ربات TeleCli اضافه شده🔘', ok_cb, false)
			end
		...
		if matches[1] == 'link' then
			...
			if not group_link then
				return "♻️Create a link using /newlink first! ♻️\n\nnoOr if I am not creator use /setlink to set your linkno"
			end
